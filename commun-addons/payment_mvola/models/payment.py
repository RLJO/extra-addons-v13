...
from odoo.addons.payment_mvola.controllers.main import MvolaController
...
class MvolaAcquirer(models.Model):
    ...
    def _get_mvola_urls(self, state):
        ...
        if state == 'test':
            return {
                ...
                'api_version': '2.0.0',
                'mvola_form_url': urls.url_join(base_url, '/payment/mvola/')
            }
        else:
            return {
                'mvola_base_url': "https://www.telma.net/mpgw/v2",
    ...
    def mvola_form_generate_values(self, values):
        ...
        #    'business': self.paypal_email_account,
            ...
            'notify_url': urls.url_join(base_url, MvolaController._notify_url),
            'mvola_return': urls.url_join(base_url, MvolaController._return_url),
            'cancel_return': urls.url_join(base_url, MvolaController._cancel_url),
        })
